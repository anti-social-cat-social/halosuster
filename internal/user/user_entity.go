...
type ITRegisterDTO struct {
	NIP      string `json:"nip" binding:"required,numeric,min=13,max=15" validate:"valid_nip"`
...
type NurseUpdateDTO struct {
	NIP  int    `json:"nip" binding:"required,numeric,min=13" validate:"valid_nip"`
	Name string `json:"name" binding:"required,min=5,max=50"`
}
